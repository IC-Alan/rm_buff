...
uint32_t Verify_CRC8_Check_Sum(const uint8_t *pchMessage, uint32_t dwLength)
{
    ...
    if ((pchMessage == nullptr) || (dwLength <= 1)) return false;

    ucExpected = Get_CRC8_Check_Sum(pchMessage, dwLength - 1, 0);
...
void Append_CRC8_Check_Sum(uint8_t * pchMessage, uint32_t dwLength)
{
    ...
    if ((pchMessage == nullptr) || (dwLength <= 1)) return;

    ucCRC = Get_CRC8_Check_Sum(pchMessage, dwLength - 1, 0);
